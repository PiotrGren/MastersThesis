...
* **Scenariusze Behawioralne:** Zaimplementowano klasy użytkowników symulujących różne strategie:
    * `WindowShopper`: "Oglądacz", marzyciel. Sprawdza ceny, symuluje zakupy, ale boi się ryzyka.
    * `ImpulsiveTrader`: Trader emocjonalny, szybki, nieregularny, popełniający błędy.
    * `CarefulTrader`: Analityk techniczny. Metodyczny, bezpieczny, powolny.
    * `IndecisiveTrader`: Bot lub niezdecydowany człowiek spamujący rynek.
    * `StrategicHolder`: Inwestor fundamentalny. Rzadka aktywność, duży wolumen, chirurgiczna precyzja.
